...
class TestPet:
    ...
    def test_get_nonexistent_pet(self):
        with allure.step("Отправка запроса на получение информацию о несуществующем питомце"):
    ...
    def test_add_new_full_pet(self):
        ...
        with allure.step("Проверка параметров питомца c полными данными в ответе"):
            assert response_json['id'] == payload['id'], "id питомца не совпадает с ожидаемым"
            assert response_json['name'] == payload['name'], "имя питомца не совпадает с ожидаемым"
            assert response_json['category'] == payload['category'], "статус питомца не совпадает с ожидаемым"
            assert response_json['photoUrls'] == payload['photoUrls'], "статус питомца не совпадает с ожидаемым"
    ...
    def test_updating_pet_information(self, create_pet):
        with allure.step("Получение ID созданного питомца"):
            pet_id = create_pet["id"]
